...
def print_extra_stats(test_inp: List[List[str]], test_out: List[str], num_cases: int) -> None:
    print(
        f" - Average length: {yellow}{sum(len(x[0].split()) for x in test_inp) // num_cases:_}"
        f"{reset}"
    )
    print(f" - Max length: {yellow}{max(len(x[0].split()) for x in test_inp):_}{reset}")
...
def test_solution_aio(test_inp: List[List[str]], test_out: List[str], num_cases: int) -> None:
    ...
    passed = i = 0
    for i, (out, inp, exp) in enumerate(
        zip_longest(output, test_inp[:num_cases], test_out[:num_cases]), 1
    ):
        if out != exp:
            print(f"Test nr:{i}\n      Input: {cyan}")
            pprint(inp)
...
def speed_test_solution_aio(
    ...
    def test_for_speed_aio(input: Callable) -> bool:
        ...
        return output == test_out[:speed_num_cases]
